<script>
  ...
  let { id = null, logo = null, name = null, employees = null, location = null, mexicanstate = null, category = null, slug = null, ...rest } = $props();
</script>

<a href="/companies/{slug}" class="no-underline text-text">
  <div class="container mx-auto px-4 py-4 border-b border-primary-light text-sm bg-white hover:bg-[#f6f3ee] focus:bg-[#f6f3ee]">
    <div class="flex flex-row items-center gap-4">
      
      <div class="w-10 min-w-10 h-full">
        {#if logo}      
          <img src="/logos/{logo}" class="w-full h-auto" alt="Logo for {name}"/>
        {/if}
      </div>

      <div class="content-start flex flex-col items-start gap-1">
        <span class="font-medium text-base">{toTitleCase(name)}</span>
        <div class="flex flex-row gap-x-2 gap-y-0 flex-wrap">
          ...
          {#if mexicanstate}            
            <span class="text-secondary">•</span>
            <span>{toTitleCase(mexicanstate)}</span>
          {/if}
        </div>
        <span class="inline-flex align-middle gap-x-1.5 py-[0.15rem] px-2 rounded-lg font-medium text-xs bg-primary-light text-[#413E37]">{toTitleCase(category)}</span>
      </div>
    </div>
  </div>
</a>